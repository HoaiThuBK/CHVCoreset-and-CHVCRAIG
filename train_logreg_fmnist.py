...
Script huấn luyện mô hình Logistic Regression trên FashionMNIST (10 lớp), có hỗ trợ nhiều
phương pháp chọn coreset (craig, chvs4, random, craig_ch) hoặc huấn luyện trên toàn bộ dataset.
"""
import numpy as np
import torch
import csv 
...
def set_seed(seed):
    """
    Hàm đặt seed cho các bộ sinh số ngẫu nhiên và bật chế độ chạy xác định (deterministic) để tái lập kết quả.
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Huấn luyện Coreset với Hồi quy Logistic trên FashionMNIST 10 lớp')
    ...
    # --- Khởi tạo mô hình ---
    print(f"Sử dụng mô hình: Logistic Regression ({input_dim} features)")
