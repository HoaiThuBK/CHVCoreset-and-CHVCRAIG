"""
---file train_resnet20_svhn.py---
Script huấn luyện mô hình ResNet-20 trên SVHN (10 lớp), có hỗ trợ nhiều phương pháp chọn
...
from utils import load_svhn_all, get_indices_by_class, train_one_epoch, evaluate
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Huấn luyện Coreset với Resnet20 trên SVHN 10 lớp')

    # --- Định nghĩa các tham số dòng lệnh ---
    parser.add_argument('--selection_method', type=str, default='craig_ch', 
    ...
    parser.add_argument('--update_freq', type=int, default=40, 
                        help='Tần suất cập nhật coreset (số epoch). Mặc định: 40')
    ...
    parser.add_argument('--warmup_epochs', type=int, default=10, 
                        help='Số epoch để khởi động tốc độ học. Mặc định: 10')
    parser.add_argument('--seed', type=int, default=42,
                        help='Random seed for reproducibility')
    parser.add_argument('--gradient_type', type=str, default="logit", choices=["logit", "embedding"],
                        help='Phương pháp biểu diễn gradient. Mặc định là embedding gradient')
    ...
    # --- Load dữ liệu SVHN ---
    trainset_full, testset = load_svhn_all()
    ...
    output_dir = os.path.join("results_svhn", args.selection_method)
    os.makedirs(output_dir, exist_ok=True)
    filename = f"results_svhn_resnet20_{args.selection_method}_{args.gradient_type}_seed{args.seed}"
    ...
    csv_file.close()
    print(f"\nHuấn luyện hoàn tất. Kết quả đã được lưu vào file: {filepath}")
